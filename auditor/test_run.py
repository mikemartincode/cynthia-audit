...
from test_author import ENTRY, GOOD_ORACLE  # noqa: E402


class ConcurrencyProbe:
    """Stub gateway: sleeps to force overlap, counts concurrent in-flight calls."""
    ...
    def __call__(self, model, system, user, *, max_tokens=0, timeout=0):
        with self._lock:
            self.active += 1
            self.calls += 1
        ...
        with self._lock:
            self.active -= 1
        return {"code": GOOD_ORACLE, "raw": GOOD_ORACLE, "in_tok": 100, "out_tok": 200,
...
def main() -> None:
    ...
    try:
        ...
        run_mod.EST_FIRST_CALL_USD = 0.02
        try:
            with tempfile.TemporaryDirectory() as td:
                run_dir = Path(td) / "run"
                ...
                assert s["completed"] == 2 and len(s["dropped_by_budget"]) == 6, s
                assert s["spent_usd"] <= 0.05, s
                assert len(list((run_dir / "records").glob("*.json"))) == 2
                print(f"budget rail ok: stopped at ${s['spent_usd']} of $0.05, "
                      f"2 recorded, {len(s['dropped_by_budget'])} dropped (logged)")

                # 3) resume: same run-dir with budget restored finishes the rest
                os.environ["AUDIT_BUDGET_USD"] = "50"
                s2 = asyncio.run(run_mod.run_audit(_manifest(8), "stub", run_dir,
                                                   concurrency=8, attempts=1))
                assert len(s2["resumed_skips"]) == 2 and s2["completed"] == 6, s2
